...
class Parameter:
    def __init__(self, D, P, ll, tt0, uu, ww, CC, KK, RR, SS):
        self.D = D
        self.P = P
        self.ll = ll
        self.tt0 = tt0
...
class Variable:
    def __init__(self, v, p, t, e, a):
        self.x0 = np.concatenate((v, p, t, e, a))
        self.len_x = self.x0.shape[0]

        self.m, self.l, self.n, self.k = v.shape[0], p.shape[0], e.shape[0], a.shape[0]
        self.start_idx = [self.m, self.m+self.l, self.m+2*self.l, self.m+2*self.l+self.n]
        self.a = a
...
class MeMo:
    def __init__(self, f: np.array, var: Variable, para: Parameter):
        ...
        """
        self.f = np.concatenate([f, np.zeros(2 * var.l + var.n + var.k)])
        self.var = var
        self.para = para
    ...
    def constraints(self, x):
        ...
        return np.concatenate([
            np.add(x[:self.var.m], np.matmul(self.para.KK, x[self.var.start_idx[2]: self.var.start_idx[3]])),
            np.matmul(self.para.SS, x[:self.var.m]),
            [np.sum(x[self.var.m: self.var.start_idx[1]])],
            np.matmul(self.para.CC, x[self.var.start_idx[2]: self.var.start_idx[3]]) - x[self.var.m: self.var.start_idx[1]],
            x[self.var.start_idx[1]: self.var.start_idx[2]] - np.divide(x[self.var.m: self.var.start_idx[1]], self.para.ww),
            x[self.var.start_idx[1]: self.var.start_idx[2]] - np.multiply(self.para.tt0, np.power(2, np.matmul(self.para.RR, self.var.a)))
        ])
    ...
    def jacobian(self, x):
        ...
        j1 = np.concatenate([
            ...
            np.zeros((self.var.m, 2 * self.var.l)),   # dp, dt
        ...
        j2 = np.concatenate([
            self.para.SS,                                                      # dv
            np.zeros((self.para.s, 2 * self.var.l + self.var.n + self.var.k))  # dp, dt, de, da
        ], axis=1)

        j3 = np.concatenate([
            np.zeros((1, self.var.m)),                             # dv
            np.ones((1, self.var.l)),                              # dp
            np.zeros((1, self.var.l + self.var.n + self.var.k))    # dt, de, da
        ], axis=1)
